...
parser.add_argument("--links_file", type=str, default="./data/sampled_links.csv")
parser.add_argument("--output_file", type=str, default="./data/movielens_movie_meta_sampled.jsonl")
...
def scrap_with_tmdbID(movieID: int, tmdbID: int, sess=None, headers=None):
    # 发送HTTP请求获取网页内容
...
def scrap_with_list(df, valid_index, s, e, queue):
    ...
    for i in valid_index[s:e]:
        ...
        if movie_id not in existing_movie_id:
            success, content = scrap_with_tmdbID(movie_id, tmdb_id, sess=None, headers=headers)
            n_success += success
            if success:
...
movie_id_df = pd.read_csv(args.links_file, header=0)[['movieId', 'tmdbId']]
...
if args.multiprocess > 1:   # 多进程
    ...
    for i in valid_index:
        ...
        if tmdb_id > 0:
            try:
                success, content = scrap_with_tmdbID(movieID=movie_id, tmdbID=tmdb_id)
                if success:
                    n_success += 1
